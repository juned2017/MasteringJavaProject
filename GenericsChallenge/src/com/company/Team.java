...
public class Team<T extends Player> implements Comparable<Team<T>>{ //(T) indicating a type paramter will be pass, type will be known when we instantiate this class during run time
    //if you had it without (extends Player) your code wuld still work but then users would be able to add anytype of class in the <> argument, like String class.
    //no we're saying we are only accepting data type that extended (Player) class
